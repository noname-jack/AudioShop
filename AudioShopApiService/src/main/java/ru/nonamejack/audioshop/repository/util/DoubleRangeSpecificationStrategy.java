...
public class DoubleRangeSpecificationStrategy implements AttributeFilterSpecificationStrategy {
    ...
    public boolean supports(AttributeValueFilter filter) {
        return filter.filterType() == FilterType.RANGE
                && filter.valueType() == ValueType.DOUBLE
                && filter.rangeValue() != null
                && filter.rangeValue().min() != null
                && filter.rangeValue().max() != null;
    }
    ...
    public Specification<Product> toSpecification(AttributeValueFilter filter) {
        return (root, query, cb) -> {
            ...
            DoubleRangeDto r = filter.rangeValue();
            return cb.and(
                    byId,
                    cb.between(join.get("doubleValue"), r.min(), r.max())
            );
        };
    }
}
